...
def classify(text):
    ...
    project_action = any(x in t for x in PROJECT_ACTIONS)
...
def is_relevant(text):
    ...
    seller_pressure = c["marketing_hits"] + c["ad_hits"]

    if c["buyer_hits"] >= 1 and seller_pressure >= 1 and not c["first_person"] and not c["trade_request"]:
        return False
    ...
    if c["first_person"] and c["project_action"] and c["concrete_hits"] >= 1:
        return True
