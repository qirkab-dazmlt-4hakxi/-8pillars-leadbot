...
def scan():
    ...
    budget=max(1,int(os.getenv("BRAVE_REQUESTS_PER_SCAN","3")))

    # Spread each scan across different cities and different query families.
    slot=int(time.time()//1800)

    pairs=[]
    for i in range(budget):
        city=CITIES[(slot+i) % len(CITIES)]
        template=BUYER_QUERY_TEMPLATES[(slot+i) % len(BUYER_QUERY_TEMPLATES)]
        pairs.append((city,template))

    for city,template in pairs:
