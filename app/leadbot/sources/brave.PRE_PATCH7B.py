...
def scan():
    ...
    pairs=[(city,template) for city in CITIES for template in BUYER_QUERY_TEMPLATES]
    budget=max(1,min(len(pairs),int(os.getenv("BRAVE_REQUESTS_PER_SCAN","3"))))

    # Rotate every 30 minutes so repeated scans cover different city/query pairs.
    slot=int(time.time()//1800)
    start=(slot*budget)%len(pairs)
    ordered=pairs[start:]+pairs[:start]

    for city,template in ordered[:budget]:
